...
<head>
    ...
    function add(){
    	$.jBox("iframe:<s:url value='/back/userAdd'/>", {
    	    title: "添加用户",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function edit(id){
    	$.jBox("iframe:<s:url value='/back/userEdit'/>?id="+id, {
    	    title: "修改用户",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function changeUserStatus(id,status){
    	...
    	$.jBox.confirm("确定 "+(status==0?"禁用":"启用")+" 用户吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("<s:url value='/back/changeUserStatus'/>",{id:id,status:status},function(responseText){
    ...
    function delUser(id,msg){
    	...
    	$.jBox.confirm("确定删除 ["+msg+"] 用户吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("<s:url value='/back/delUser'/>",{id:id},function(responseText){
    	    		if(responseText==true){
    	    			$.jBox.info("操作成功，请刷新查看结果", "成功信息",{top: '20%'});
    	    			//window.location.reload();
    	    		}else{
    ...
    function delUsers(){
    	...
    	$.jBox.confirm("确定批量删除吗？", "批量操作确认", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("/back/delUsers",{userIds:selectedItems.join(',')},function(data){
    	    		if(data>0){
    ...
    function userRoleSet(userId){
    	$.jBox("iframe:<s:url value='/back/userRoleSet'/>?userId="+userId, {
    	    title: "用户授权",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
                    ...
                    <c:choose>
						...
						<td class="operation">
							<a href="javascript:;" onclick="edit(${dataItem.id});">
							 	<img src="<s:url value='/resources/css/images/operation/pencil.png'/>" title="修改用户信息"/>
							 </a>
							 
							 <c:choose>
								<c:when test="${dataItem.status==0}">
									 <a href="javascript:;" onclick="changeUserStatus(${dataItem.id},1);">
		                            	<img src="<s:url value='/resources/css/images/operation/lock_unlock.png'/>" title="启用用户"/>
		                            </a>
								</c:when>					
								<c:otherwise>
		                            <a href="javascript:;" onclick="changeUserStatus(${dataItem.id},0);">
		                            	<img src="<s:url value='/resources/css/images/operation/lock.png'/>" title="禁用用户"/>
		                            </a>
								</c:otherwise>
							</c:choose>
							
							 <a href="javascript:;" onclick="userRoleSet(${dataItem.id});">
							 	<img src="<s:url value='/resources/css/images/operation/member.png'/>" title="用户权限"/>
							 </a>   
							 <a href="javascript:;" onclick="delUser(${dataItem.id},'${dataItem.userName}');">
							 	<img src="<s:url value='/resources/css/images/operation/trashcan_delete.png'/>" title="删除用户"/>
							 </a>
