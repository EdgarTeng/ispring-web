...
<head>
    ...
    function add(){
    	$.jBox("iframe:<s:url value='/back/roleAdd'/>", {
    	    title: "添加角色",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function edit(id){
    	$.jBox("iframe:<s:url value='/back/roleEdit'/>?id="+id, {
    	    title: "修改角色",
    	    width: 700,
    	    height: 320,
    	    buttons: {}
    ...
    function delRole(id,msg){
    	...
    	$.jBox.confirm("确定删除 ["+msg+"] 角色吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("<s:url value='/back/delRole'/>",{id:id},function(responseText){
    ...
    function changeRoleStatus(id,status){
    	...
    	$.jBox.confirm("确定 "+(status==0?"禁用":"启用")+" 用户吗？", "确认操作", function (v, h, f) {
    	    if (v == 'ok'){
    	    	...
    	    	$.post("<s:url value='/back/changeRoleStatus'/>",{id:id,status:status},function(responseText){
    	    		if(responseText==true){
    	    			$.jBox.info("操作成功，请刷新查看结果", "成功信息",{top: '20%'});
    	    			//window.location.reload();
    	    		}else{
    ...
    function roleResourceSet(roleId){
    	$.jBox("iframe:<s:url value='/back/roleResourceSet'/>?roleId="+roleId, {
    	    title: "角色资源授权",
    	    width: 700,
    	    height: 550,
    	    buttons: {}
                    ...
                    <c:choose>
						...
						<td class="operation">
							<a href="javascript:;" onclick="edit(${dataItem.id});">
							 	<img src="<s:url value='/resources/css/images/operation/pencil.png'/>" title="修改角色信息"/>
							 </a>
							 
							 <c:choose>
								<c:when test="${dataItem.status==0}">
									 <a href="javascript:;" onclick="changeRoleStatus(${dataItem.id},1);">
		                            	<img src="<s:url value='/resources/css/images/operation/lock_unlock.png'/>" title="启用角色"/>
		                            </a>
								</c:when>					
								<c:otherwise>
		                            <a href="javascript:;" onclick="changeRoleStatus(${dataItem.id},0);">
		                            	<img src="<s:url value='/resources/css/images/operation/lock.png'/>" title="禁用角色"/>
		                            </a>
								</c:otherwise>
							</c:choose>
							           
							 <a href="javascript:;" onclick="roleResourceSet(${dataItem.id});">
							 	<img src="<s:url value='/resources/css/images/operation/folder.png'/>" title="角色权限"/>
							 </a>
							 <a href="javascript:;" onclick="delRole(${dataItem.id},'${dataItem.roleName}');">
							 	<img src="<s:url value='/resources/css/images/operation/trashcan_delete.png'/>" title="删除角色"/>
							 </a>
						</td>
